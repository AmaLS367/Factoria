...
function CredibilityBadge({ score }: { score: number | null }) {
  if (score === null) {
    return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-neutral-100 text-neutral-500 dark:bg-neutral-700 dark:text-neutral-400">?</span>;
  }
  if (score >= 0.7) {
    return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400">High</span>;
  }
  if (score >= 0.45) {
    return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400">Med</span>;
  }
  return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400">Low</span>;
}

function SourcesPanel({ identifierValue, onClose }: { identifierValue: string; onClose: () => void }) {
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-200 dark:border-neutral-700">
          <h3 className="text-sm font-semibold text-neutral-800 dark:text-neutral-100">
            Sources — <span className="font-normal text-neutral-500">{identifierValue}</span>
          </h3>
        ...
        <div className="overflow-y-auto flex-1 p-4">
          ...
          {!loading && !error && sources.length === 0 && (
            <p className="text-neutral-500 dark:text-neutral-400 text-sm text-center py-8">No sources found.</p>
          )}
          {!loading && sources.map((src, i) => (
            <div key={i} className="mb-3 p-3 rounded border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800">
              ...
              {src.title && (
                <p className="text-xs text-neutral-500 dark:text-neutral-400 truncate mt-0.5">{src.url}</p>
              )}
              {src.snippet && (
                <p className="text-xs text-neutral-600 dark:text-neutral-300 mt-1 line-clamp-2">{src.snippet}</p>
              )}
              <div className="flex gap-2 mt-1.5 text-xs text-neutral-400">
                {src.provider && src.provider !== 'legacy' && (
                  <span className="px-1.5 py-0.5 rounded bg-neutral-200 dark:bg-neutral-700">{src.provider}</span>
                )}
...
export function ItemsTable() {
  ...
  const columns = items.length > 0 ? Object.keys(items[0]).filter(c => c !== 'Sources') : [];
  ...
  return (
    <div className="space-y-4">
      {openSourcesFor && (
        <SourcesPanel identifierValue={openSourcesFor} onClose={() => setOpenSourcesFor(null)} />
      )}
        ...
        <div className="flex justify-between items-center px-4 py-3 border-b border-neutral-200 dark:border-neutral-700 bg-neutral-50/50 dark:bg-neutral-800/50">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex space-x-1 ml-4">
              <button onClick={() => setOffset(offset - PAGE_SIZE)} disabled={offset === 0 || loading}
              ...
              <button onClick={() => setOffset(offset + PAGE_SIZE)} disabled={items.length < PAGE_SIZE || loading}
                className="p-1 rounded text-neutral-500 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-700 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
              <thead className="bg-neutral-50 dark:bg-neutral-800">
                <tr>
                  ...
                  {hasIdentifier && (
                    <th scope="col" className="px-4 py-3 text-left text-xs font-semibold text-neutral-600 dark:text-neutral-300 uppercase tracking-wider">
                      Sources
                    </th>
                  )}
                </tr>
              ...
              <tbody className="bg-white dark:bg-neutral-900 divide-y divide-neutral-200 dark:divide-neutral-700 text-sm">
                {items.map((row, i) => {
                  const firstVal = columns.length > 0 ? String(row[columns[0]] ?? '') : '';
                  return (
                    <tr key={i} className="hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20 transition-colors">
                      ...
                      {hasIdentifier && (
                        <td className="px-4 py-2">
                          <button
                            onClick={() => setOpenSourcesFor(firstVal)}

...
class ScriptedLLM:
    ...
    def get_answer(self, prompt: str) -> tuple[str, TokenUsage]:
        self.calls += 1
        return self.answers.pop(0) if self.answers else "", TokenUsage(), TokenUsage()
...
def main() -> int:
    with TemporaryDirectory() as tmp:
        ...
        values, conf, _ = agent.collect_item_with_confidence("ITEM-001", ["Name"])
        ...
        agent = ResearchAgent(llm_client=llm, search_tool=EmptySearch())
        values, conf, _ = agent.collect_item_with_confidence("ITEM-002", ["Name"])
        print(f"\nResult: values={values} conf={conf} calls={llm.calls}")
        ...
        agent = ResearchAgent(llm_client=llm, search_tool=EmptySearch())
        values, conf, _ = agent.collect_item_with_confidence("ITEM-003", ["Name"])
        print(f"\nResult: values={values} conf={conf} calls={llm.calls}")
